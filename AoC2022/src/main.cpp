//#include "AoC_day01.h"
//#include "AoC_day02.h"
//#include "AoC_day03.h"
//#include "AoC_day04.h"
//#include "AoC_day05.h"
//#include "AoC_day06.h"
//#include "AoC_day07.h"
//#include "AoC_day08.h"
//#include "AoC_day09.h"
//#include "AoC_day10.h"
//#include "AoC_day11.h"
//#include "AoC_day12.h"
//#include "AoC_day14.h"
//#include "AoC_day15.h"
//#include "AoC_day21.h"
#include "AoC_day23.h"
//#include "AoC_day25.h"

int main()
{
	std::string path = "C:/Users/dacaz/source/repos/AoC2022/AoC2022/data/";

	//AoC22_01 day01(path + "data01.txt");
	//AoC22_02 day02(path + "data02.txt");
	//AoC22_03 day03(path + "data03.txt");
	//AoC22_04 day04(path + "data04.txt");
	//AoC22_05 day05(path + "data05.txt");
	//AoC22_06 day06(path + "data06.txt");
	//AoC22_07 day07(path + "data07.txt");
	//AoC22_08 day08(path + "data08.txt");
	//AoC22_09 day09(path + "data09.txt");
	//AoC22_10 day10(path + "data10.txt");
	//AoC22_11 day11(path + "data11.txt");
	//AoC22_12 day12(path + "data12.txt");
	//AoC22_14 day14(path + "data14.txt");
	//AoC22_15 day15(path + "data15.txt");
	//AoC22_21 day21(path + "data21.txt");
	AoC22_23 day23(path + "data23.txt");
	//AoC22_25 day25(path + "data25.txt");
}